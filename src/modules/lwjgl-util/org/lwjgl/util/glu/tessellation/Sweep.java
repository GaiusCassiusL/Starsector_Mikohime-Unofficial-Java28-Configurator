...
class Sweep {
   ...
   static boolean CheckForIntersect(GLUtessellatorImpl tess, ActiveRegion regUp) {
      ...
      if ((Geom.VertEq(dstUp, tess.event) || !(Geom.EdgeSign(dstUp, tess.event, isect) >= 0.0))
   ...
   static void DoneEdgeDict(GLUtessellatorImpl tess) {
      ...
      while ((reg = (ActiveRegion)Dict.dictKey(Dict.dictMin(tess.dict))) != null) {
         if (!reg.sentinel) {
            assert reg.fixUpperEdge;
            // Decompiler artifact fix: Vineflower failed to resugar this back
            // into `assert ++fixedEdges == 1;` (the increment is a side
            // effect inside the assert condition, only evaluated when
            // assertions are enabled) and instead emitted a raw reference to
            // the compiler-synthesized $assertionsDisabled field, which is
            // not visible to source code. Restored as a real assert
            // statement so semantics (including the conditional increment)
