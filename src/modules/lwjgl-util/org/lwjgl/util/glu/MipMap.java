...
public class MipMap extends Util {
   ...
   public static int gluScaleImage(
      ...
      switch (typein) {
         ...
         // Decompiler artifact fix: both case bodies declared `int k = 0;`
         // in the same switch scope (bytecode local-slot reuse); wrapped
         // each case in its own block so the two declarations don't collide.
         case 5126: {
            int k = 0;
            ((Buffer)dataIn).rewind();
