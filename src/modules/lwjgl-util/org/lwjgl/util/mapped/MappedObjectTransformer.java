...
public class MappedObjectTransformer {
   ...
   private static MappedObjectTransformer.FieldInfo registerField(boolean autoGenerateOffsets, String className, long advancingOffset, Field field) {
      ...
      if (pointer != null && field.getType() != long.class) {
         throw new ClassFormatError(
            "The @Pointer annotation can only be used on long fields. @Pointer field found: " + className + "." + field.getName() + ": " + field.getType()
         );
      }
   ...
   static int transformMethodCall(
      ...
      int i,
      Map<AbstractInsnNode, Frame> frameMap,
      ...
      switch (methodInsn.getOpcode()) {
         case 182:
            if ("asArray".equals(methodInsn.name) && methodInsn.desc.equals("()[L" + MAPPED_OBJECT_JVM + ";")) {
               ...
               Frame frame = frameMap.get(var11);
               ...
               ? new FieldNode(access, name, desc, signature, value) {
                  public void visitEnd() {
                     if (this.visibleAnnotations == null) {
                        ...
                        // Decompiler artifact fix: ASM 3.3.1's FieldNode.visibleAnnotations
                        // field is a raw (non-generic) List, so the loop variable must be
                        // Object with an explicit cast rather than a generic for-each.
                        for (Object annotationObj : this.visibleAnnotations) {
                           AnnotationNode pad = (AnnotationNode)annotationObj;
                        ...
                        if (before) {
                           ...
                           for (int i = count; i >= 1; i--) {
                              TransformationAdapter.this.cv.visitField(this.access | 1 | 4096, this.name + "$PAD_" + i, this.desc, this.signature, null);
                           }
                        }
                        ...
                        if (after) {
                           ...
                           for (int i = 1; i <= count; i++) {
                              TransformationAdapter.this.cv.visitField(this.access | 1 | 4096, this.name + "$PAD" + i, this.desc, this.signature, null);
                           }
                        }
                     }
                  }
