...
public final class Display {
   ...
   public static DisplayMode setDisplayMode(DisplayMode[] dm, final String[] param) throws Exception {
      // Decompiler artifact fix: Vineflower emitted the FieldAccessor local
      // class declaration after the (unreachable, dead-code) `throw` at the
      // end of this method, but Sorter (declared next) references
      // FieldAccessor before that point. Java requires a local class to be
      // declared before its first use in the same block, so FieldAccessor is
      // moved here, ahead of Sorter. No behavior changes: local class
      // declaration order has no runtime effect.
      class FieldAccessor {
