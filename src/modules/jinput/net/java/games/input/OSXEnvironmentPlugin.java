...
public final class OSXEnvironmentPlugin extends ControllerEnvironment implements Plugin {
   ...
   static String getPrivilegedProperty(String property) {
      ...
      return (String) AccessController.doPrivileged(new OSXEnvironmentPlugin$2(property));
   }

   static String getPrivilegedProperty(String property, String default_value) {
      // decompiler artifact: doPrivileged returns Object; cast to expected return type
      return (String) AccessController.doPrivileged(new OSXEnvironmentPlugin$3(property, default_value));
   }
   ...
   private static final void addElements(OSXHIDQueue queue, List elements, List components, boolean map_mouse_buttons) throws IOException {
      // decompiler artifact: raw List requires unchecked cast for enhanced for loop
      for (OSXHIDElement element : (java.util.List<OSXHIDElement>) elements) {
