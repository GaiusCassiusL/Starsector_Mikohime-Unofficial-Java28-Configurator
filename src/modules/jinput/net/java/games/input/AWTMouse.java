...
final class AWTMouse extends Mouse implements AWTEventListener {
   ...
   private final Deque<AWTEvent> awt_events = new ArrayDeque<>();
   private final Deque<AWTEvent> processed_awt_events = new ArrayDeque<>();
   ...
   public final synchronized void pollDevice() throws IOException {
      ...
      AWTEvent event;
      while ((event = this.awt_events.pollFirst()) != null) {
         this.processEvent(event);
         this.processed_awt_events.addLast(event);
      }
   }

   protected final synchronized boolean getNextDeviceEvent(Event event) throws IOException {
      while (!this.processed_awt_events.isEmpty()) {
         AWTEvent awt_event = this.processed_awt_events.peekFirst();
         if (awt_event instanceof MouseWheelEvent) {
            ...
            this.processed_awt_events.pollFirst();
