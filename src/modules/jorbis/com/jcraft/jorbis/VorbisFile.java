...
public class VorbisFile {
   ...
   private final Page fetchHeadersPage = new Page();
   private final Packet fetchHeadersPacket = new Packet();
   private final int[] serialnoScratch = new int[1];
   private final Page processPage = new Page();
   private final Packet processPacketScratch = new Packet();
   private final Page pcmSeekPage = new Page();
   private final float[][][] pcmScratch = new float[1][][];
   private int[] pcmIndexScratch = new int[0];
   ...
   public VorbisFile(String file) throws JOrbisException {
      ...
      try {
         ...
         if (ret != 0) {
            throw new JOrbisException("VorbisFile: open return " + ret);
         }
      } catch (Exception e) {
         if (is != null) {
            try {
   ...
   private int[] getPcmIndexScratch(int channels) {
      if (this.pcmIndexScratch.length < channels) {
         this.pcmIndexScratch = new int[channels];
      }

      return this.pcmIndexScratch;
   }
   ...
   int fetch_headers(Info vi, Comment vc, int[] serialno, Page og_ptr) {
      Page og = this.fetchHeadersPage;
      Packet op = this.fetchHeadersPacket;
      if (og_ptr == null) {
   ...
   int open_seekable() throws JOrbisException {
      ...
      Page og = new Page();
      int ret = this.fetch_headers(initial_i, initial_c, this.serialnoScratch, null);
      int serialno = this.serialnoScratch[0];
   ...
   int open_nonseekable() {
      ...
      if (this.fetch_headers(this.vi[0], this.vc[0], this.serialnoScratch, null) == -1) {
         return -1;
      }

      this.current_serialno = this.serialnoScratch[0];
   ...
   int process_packet(int readp) {
      Page og = this.processPage;

      while (true) {
         if (this.decode_ready) {
            Packet op = this.processPacketScratch;
            int result = this.os.packetout(op);
            if (result > 0) {
               ...
               if (this.vb.synthesis(op) == 0) {
                  ...
                  this.samptrack = this.samptrack + (this.vd.synthesis_pcmout(null, null) - oldsamples);
         ...
         if (!this.decode_ready) {
            if (!this.seekable) {
               int ret = this.fetch_headers(this.vi[0], this.vc[0], this.serialnoScratch, og);
               this.current_serialno = this.serialnoScratch[0];
               if (ret != 0) {
                  return ret;
               }
   ...
   public int pcm_seek(long pos) {
      ...
      if (pos >= 0L && pos <= total) {
         ...
         while (this.pcm_offset < pos) {
            ...
            float[][][] _pcm = this.pcmScratch;
            int[] _index = this.getPcmIndexScratch(this.getInfo(-1).channels);
   ...
   int read(byte[] buffer, int length, int bigendianp, int word, int sgned, int[] bitstream) {
      ...
      while (true) {
         if (this.decode_ready) {
            Info info = this.getInfo(-1);
            float[][][] _pcm = this.pcmScratch;
            int[] _index = this.getPcmIndexScratch(info.channels);
            int samples = this.vd.synthesis_pcmout(_pcm, _index);
            float[][] pcm = _pcm[0];
            if (samples != 0) {
