...
class Floor1 extends FuncFloor {
   ...
   Object unpack(Info vi, Buffer opb) {
      ...
      if (info.partitions < 0 || info.partitions > info.partitionclass.length) {
         info.free();
         return null;
      }
      ...
      for (int j = 0; j < info.partitions; j++) {
         ...
         if (info.partitionclass[j] < 0 || info.partitionclass[j] >= info.class_dim.length) {
            info.free();
            return null;
         }
      ...
      for (int j = 0; j < maxclass + 1; j++) {
         int classDim = opb.read(3);
         if (classDim < 0) {
            info.free();
            return null;
         }

         info.class_dim[j] = classDim + 1;
         info.class_subs[j] = opb.read(2);
         if (info.class_subs[j] < 0 || (1 << info.class_subs[j]) > info.class_subbook[j].length) {
            info.free();
            return null;
         }
         ...
         for (int k = 0; k < 1 << info.class_subs[j]; k++) {
            int subbook = opb.read(8);
            if (subbook < 0) {
               info.free();
               return null;
            }

            info.class_subbook[j][k] = subbook - 1;
      ...
      if (mult < 0 || rangebits < 0) {
         info.free();
         return null;
      }

      info.mult = mult + 1;
      if (info.mult < 1 || info.mult > 4) {
         info.free();
         return null;
      }

      int maxposit = 1 << rangebits;
      int posts = 2;
      for (int j = 0; j < info.partitions; j++) {
         int clss = info.partitionclass[j];
         int cdim = info.class_dim[clss];
         if (cdim <= 0 || posts > info.postlist.length - cdim) {
            info.free();
            return null;
         }

         for (int k = 0; k < cdim; k++) {
            int t = opb.read(rangebits);
            if (t < 0 || t >= maxposit) {
               info.free();
               return null;
            }

            info.postlist[posts++] = t;
         }
      }

      info.postlist[0] = 0;
      info.postlist[1] = maxposit;
      return info;
   }

   private int postCount(Floor1.InfoFloor1 info) {
      if (info.partitions < 0 || info.partitions > info.partitionclass.length) {
         return -1;
      }
      ...
      for (int j = 0; j < info.partitions; j++) {
         int clss = info.partitionclass[j];
         if (clss < 0 || clss >= info.class_dim.length) {
            return -1;
         }
   ...
   Object look(DspState vd, InfoMode mi, Object i) {
      ...
      Floor1.InfoFloor1 info = (Floor1.InfoFloor1)i;
      int _n = this.postCount(info);
      if (_n < 2 || _n > info.postlist.length || info.postlist[1] <= 0 || info.postlist[1] > vd.vi.blocksizes[mi.blockflag] / 2) {
         return null;
      }
      ...
      look.posts = _n;
      for (int j = 0; j < _n; j++) {
         if (info.postlist[j] < 0 || info.postlist[j] > look.n) {
            return null;
         }
      }
      ...
      for (int jxx = 0; jxx < _n; jxx++) {
         ...
         if (jxx != 0 && look.sorted_index[jxx] == look.sorted_index[jxx - 1]) {
            return null;
         }
      }
   ...
   Object inverse1(Block vb, Object ii, Object memo) {
      ...
      int flag = vb.opb.read(1);
      if (flag != 1) {
         return null;
      }

      if (look.posts < 2 || look.posts > info.postlist.length || look.quant_q <= 1) {
         return null;
      }

      int quantBits = Util.ilog(look.quant_q - 1);
      if (quantBits <= 0) {
         return null;
      }

      int first = vb.opb.read(quantBits);
      int second = vb.opb.read(quantBits);
      if (first < 0 || second < 0 || first >= look.quant_q || second >= look.quant_q) {
         return null;
      }
      ...
      while (i < info.partitions) {
         ...
         if (cdim <= 0 || csubbits < 0 || csubbits > 3 || j > look.posts - cdim) {
            return null;
         }

         int csub = 1 << csubbits;
         if (csub > info.class_subbook[clss].length) {
            return null;
         }

         int cval = 0;
         if (csubbits != 0) {
            int classBook = info.class_book[clss];
            if (classBook < 0 || classBook >= books.length || books[classBook] == null) {
               return null;
            }

            cval = books[classBook].decode(vb.opb);
            if (cval == -1) {
               return null;
            }
         }
         ...
         for (int k = 0; k < cdim; k++) {
            ...
            cval >>>= csubbits;
            if (book >= 0) {
               if (book >= books.length || books[book] == null) {
                  return null;
               }
      ...
      for (int ix = 2; ix < look.posts; ix++) {
         ...
         if (j < 0 || j >= look.quant_q) {
            return null;
         }

         int hiroom = look.quant_q - j;
         int loroom = j;
         ...
         if (val != 0) {
            ...
            fit_value[ix] = val + j;
            if (fit_value[ix] < 0 || fit_value[ix] >= look.quant_q) {
               return null;
            }
   ...
   int inverse2(Block vb, Object i, Object memo, float[] out) {
      ...
      if (memo != null) {
         ...
         if (fit_value.length < look.posts || look.quant_q <= 0 || fit_value[0] < 0 || fit_value[0] >= look.quant_q) {
            return 0;
         }
         ...
         if (ly < 0 || ly >= FLOOR_fromdB_LOOKUP.length) {
            return 0;
         }
         ...
         for (int j = 1; j < look.posts; j++) {
            ...
            if (hy < 0 || hy >= look.quant_q) {
               return 0;
            }

            if (hy == fit_value[current]) {
               hy *= info.mult;
               if (hy < 0 || hy >= FLOOR_fromdB_LOOKUP.length) {
                  return 0;
               }
   ...
   private static void render_line(int x0, int x1, int y0, int y1, float[] d) {
      ...
      for (d[x] *= FLOOR_fromdB_LOOKUP[y]; ++x < x1; d[x] *= FLOOR_fromdB_LOOKUP[y]) {
         err += ady;
         if (err >= adx) {
            err -= adx;
            y += sy;
