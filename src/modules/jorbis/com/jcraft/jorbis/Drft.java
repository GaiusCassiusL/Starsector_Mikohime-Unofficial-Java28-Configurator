...
class Drft {
   ...
   static void dradf4(int ido, int l1, float[] cc, float[] ch, float[] wa1, int index1, float[] wa2, int index2, float[] wa3, int index3) {
      ...
      for (int k = 0; k < l1; k++) {
         ...
         int var46;
         ch[(var46 = t5 + (ido << 1)) - 1] = cc[t3] - cc[t4];
         ch[var46] = cc[t2] - cc[t1];
         t1 += ido;
         t2 += ido;
         t3 += ido;
         t4 += ido;
   ...
   static void dradfg(int ido, int ip, int l1, int idl1, float[] cc, float[] c1, float[] c2, float[] ch, float[] ch2, float[] wa, int index) {
      ...
      while (true) {
         switch (state) {
            // decompiler artifact: duplicate local var declarations across switch cases
            case 101: {
               if (ido == 1) {
                  state = 119;
                  break;
   ...
   static void dradb4(int ido, int l1, float[] cc, float[] ch, float[] wa1, int index1, float[] wa2, int index2, float[] wa3, int index3) {
      ...
      for (int k = 0; k < l1; k++) {
         ...
         ch[t5] = tr2 + tr3;
         int var46;
         ch[var46 = t5 + t0] = tr1 - tr4;
         ch[t5 = var46 + t0] = tr2 - tr3;
         int var48;
         ch[var48 = t5 + t0] = tr1 + tr4;
         t1 += ido;
         t3 += t2;
      }
      ...
      if (ido >= 2) {
         if (ido != 2) {
            ...
            for (int var35 = 0; var35 < l1; var35++) {
               ...
               for (int i = 2; i < ido; i += 2) {
                  ...
                  int var54;
                  ch[(var54 = t8 + t0) - 1] = wa2[index2 + i - 2] * cr3 - wa2[index2 + i - 1] * ci3;
                  ch[var54] = wa2[index2 + i - 2] * ci3 + wa2[index2 + i - 1] * cr3;
                  ch[(t8 = var54 + t0) - 1] = wa3[index3 + i - 2] * cr4 - wa3[index3 + i - 1] * ci4;
         ...
         for (int var36 = 0; var36 < l1; var36++) {
            ...
            ch[t5] = tr2 + tr2;
            int var51;
            ch[var51 = t5 + t0] = sqrt2 * (tr1 - ti1);
            ch[t5 = var51 + t0] = ti2 + ti2;
            int var53;
            ch[var53 = t5 + t0] = -sqrt2 * (tr1 + ti1);
            t3 += ido;
            t1 += t2;
            t4 += t2;
         }
